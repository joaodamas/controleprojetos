...
function renderImprovementLogList(modal, improvement) {
  ...
  if (!logs.length) {
    list.innerHTML = `<div class="imprm-log-empty">Sem entradas no log.</div>`;
    return;
  }
  const rows = logs
    .slice()
...
function usesTenantData() {
  // Unify persistence for all roles in the same data root ("clients"),
  // avoiding split state between tenants/* and clients/*.
  return false;
}
...
function progressPctFromTasks(tasks = []) {
  ...
  const avg = leaf.reduce((sum, t) => sum + taskProgressValue(t), 0) / leaf.length;
...
function renderMonitorActivitiesV2(container) {
  ...
  const listItems = filter === "proximo" ? upcomingScope : overdueScope;
  const listTitle = filter === "proximo" ? "Proximas (7 dias)" : "Intervencao imediata";
  const countLabel =
    filter === "proximo" ? `${listItems.length} atividades proximas` : `${listItems.length} atividades em atraso`;
  ...
  const renderMonitorItem = (item) => {
    ...
    const badgeText = diff < 0 ? `ATRASO: ${Math.abs(diff)}D` : diff === 0 ? "VENCE HOJE" : `EM ${diff}D`;
    ...
    return `
      <div class="monitor-v2-item monitor-v2-item--${tone}" data-monitor-card="true" data-client-index="${item.clientIndex}" data-project-index="${item.projectIndex}" data-task-index="${item.taskIndex}">
        <div class="monitor-v2-item-left">
          <label class="monitor-v2-check" data-monitor-toggle="true" title="Concluir">
          ...
          <div class="monitor-v2-body">
            ...
            <div class="monitor-v2-owner">${owner}</div>
          </div>
        </div>
  ...
  const leftLimit = 4;
  const sliced = listItems.slice(0, leftLimit);
  const extra = Math.max(0, listItems.length - sliced.length);

  const teamMap = new Map();
  overdueScope.forEach((item) => {
    const name = item.responsible || "A definir";
    const entry = teamMap.get(name) || { name, overdue: 0 };
    entry.overdue += 1;
    teamMap.set(name, entry);
  });
  const teamItems = Array.from(teamMap.values()).sort((a, b) => b.overdue - a.overdue).slice(0, 4);
  const maxOverdue = Math.max(1, ...teamItems.map((x) => x.overdue));

  const teamHtml = teamItems.length
    ? teamItems
        .map((m) => {
          const initials = escapeHtml(initialsFromName(m.name || "T"));
          const name = escapeHtml(m.name || "A definir");
          const pct = Math.round((m.overdue / maxOverdue) * 100);
          return `
            <div class="monitor-v2-team-row">
              <div class="monitor-v2-team-left">
                <span class="monitor-v2-team-avatar">${initials}</span>
                <span class="monitor-v2-team-name">${name}</span>
              </div>
              <div class="monitor-v2-team-bar" aria-hidden="true"><i style="width:${pct}%"></i></div>
              <div class="monitor-v2-team-count">${m.overdue} atrasadas</div>
            </div>
          `;
        })
        .join("")
    : `<div class="monitor-v2-empty">Sem atrasos.</div>`;

  const todayHtml = todayScope.length
    ? todayScope
        .slice(0, 3)
        .map((item) => {
          const client = escapeHtml(item.clientName || "Cliente");
          ...
          const title = escapeHtml(item.taskTitle || "Atividade");
          return `
            <div class="monitor-v2-today-item" data-monitor-card="true" data-client-index="${item.clientIndex}" data-project-index="${item.projectIndex}" data-task-index="${item.taskIndex}">
              <div class="monitor-v2-today-top">
                <span class="monitor-v2-today-client">${client}</span>
                <span class="monitor-v2-badge monitor-v2-badge--ok">VENCE HOJE</span>
              </div>
              <div class="monitor-v2-today-title">${title}</div>
              <div class="monitor-v2-today-meta">${project} | ${epic}</div>
            </div>
          `;
        })
        .join("")
    : `<div class="monitor-v2-empty monitor-v2-empty--dark">Sem entregas hoje.</div>`;

  container.innerHTML = `
    <div class="monitor-v2 span-all">
      <div class="monitor-v2-top">
        <div class="monitor-v2-head">
          <div class="monitor-v2-kicker">Monitor de Operacoes</div>
          <div class="monitor-v2-sub">Gestao de intervencoes imediatas e capacidade de time</div>
        </div>
      ...
      <div class="monitor-v2-main">
        <section class="monitor-v2-left">
          <div class="monitor-v2-section-head">
            <div class="monitor-v2-section-title">
              <span class="monitor-v2-section-ico" aria-hidden="true">
                <svg viewBox="0 0 24 24"><path d="M10.29 3.86L1.82 18a2 2 0 0 0 1.71 3h16.94a2 2 0 0 0 1.71-3L13.71 3.86a2 2 0 0 0-3.42 0z"></path><line x1="12" y1="9" x2="12" y2="13"></line><line x1="12" y1="17" x2="12.01" y2="17"></line></svg>
              </span>
              <span>${escapeHtml(listTitle)}</span>
            </div>
            <span class="monitor-v2-count">${escapeHtml(countLabel)}</span>
          </div>
          <div class="monitor-v2-list">
            ${sliced.length ? sliced.map(renderMonitorItem).join("") : `<div class="monitor-v2-empty">Sem atividades.</div>`}
          </div>
          ${extra > 0 ? `<div class="monitor-v2-more">+${extra} atividades restantes</div>` : ""}
        </section>

        <aside class="monitor-v2-right">
          <div class="monitor-v2-side-card">
            <div class="monitor-v2-side-head">
              <div class="monitor-v2-side-title">Carga por responsavel</div>
            </div>
            <div class="monitor-v2-team">
              ${teamHtml}
            </div>
          </div>

          <div class="monitor-v2-side-card monitor-v2-side-card--dark">
            <div class="monitor-v2-side-head">
              <div class="monitor-v2-side-title">Entregas do dia</div>
            </div>
            <div class="monitor-v2-today">
              ${todayHtml}
            </div>
            <button type="button" class="monitor-v2-next7" data-monitor-view-next7>Ver proximos 7 dias</button>
          </div>
        </aside>
      </div>
    </div>
...
function renderHomeMacroSummary() {
  ...
  const buildAlert = (project) => {
    ...
    for (const t of tasks) {
      ...
      if (days > worstOverdueDays) {
        worstOverdueDays = days;
        worstTitle = taskTitle(t);
      }
    }
    if (!overdueCount) return "Nenhuma pendencia critica hoje";
    if (worstOverdueDays && worstTitle) return `Maior atraso: ${worstOverdueDays}d - ${worstTitle}`;
    ...
    return `
      <article class="home-project-card" data-home-open-project="true" data-client-name="${escapeHtml(clientName)}" data-project-name="${escapeHtml(projectName)}">
        ...
        <div class="home-project-alert">
          ...
          <div class="home-project-alert-text">${alertText}</div>
        </div>
    ...
    <div class="home-v2">
      <div class="home-v2-top">
        ...
        <div class="home-v2-actions">
          ...
          <div class="home-v2-user-pill" title="${escapeHtml(state.currentUserEmail || "Usuario")}">
            <span>${escapeHtml(initialsFromName(state.currentUserEmail || "U"))}</span>
          </div>
        </div>
      </div>
...
function renderImprovementsDashboard(container) {
  ...
  const kpis = [
    { label: "Backlog ativo", value: total, icon: "sparkles" },
    { label: "Em triagem", value: triagem, icon: "clock" },
    { label: "Em execucao", value: emExecucao, icon: "wand-2" },
    { label: "Concluidos", value: concluido, icon: "check-circle-2" },
    { label: "Sucesso mensal", value: "94%", icon: "trending-up" }
  ];
  ...
  const stageColumns = IMPROVEMENT_STAGES.map((st) => {
    ...
    const cards = items
      .map((it) => {
        ...
        const sub = escapeHtml(it?.client || "-");
        ...
        return `
          <button type="button" class="impr-card impr-card--stage-${escapeHtml(stageId)}" data-improvement-card="${escapeHtml(String(it.id || ""))}">
            <div class="impr-card-top">
              <span class="impr-pill impr-pill--id impr-pill--stage-${escapeHtml(stageId)}">${idLabel}</span>
              <span class="impr-pill impr-pill--pct">${pct}%</span>
            </div>
            <div class="impr-card-title">${title}</div>
            <div class="impr-card-sub">${sub}</div>
            <div class="impr-card-bar"><i style="width:${pct}%"></i></div>
            <div class="impr-card-foot">
              <span class="impr-avatar">${initials}</span>
              <span class="impr-exec">${execLabel}</span>
              <span class="impr-arrow">›</span>
            </div>
          </button>
        `;
      })
    ...
    return `
      <section class="impr-col" data-improvement-stage="${escapeHtml(st.id)}">
        ...
        <div class="impr-col-body">
          ${cards || `<div class="impr-empty">Sem itens.</div>`}
        </div>
      </section>
    `;
  });
...
function renderImprovementDetail(container) {
  ...
  const originLabel = improvement.origin === "chamado" ? "Chamado" : "E-mail";
...
function buildOnePagePayload(project, client) {
  ...
  return {
    project: {
      ...
      // Keep leader and developer separate for the OnePage header.
      leader: leaderLabel,
      developer: developersLabel,
      developers: devList,
      team: Array.isArray(project?.team) ? project.team : [],
      // Macro hierarchy (used to disambiguate tasks with same title in OnePage cards).
      ...
      // Back-compat: some OnePage blocks still read `responsible`.
...
function renderMain() {
  ...
  // Previsto (baseline): preferir baseline por tarefa (quando existir); fallback para linear por tempo.
  const plannedByTasks = computePlannedBaselinePctByTasks(selectedProject, tasks);
  const sCurveSeries = plannedByTasks == null ? computeSCurveDailyBaseline(selectedProject, tasks || null, progressPct) : null;
  const baselinePct = plannedByTasks != null
    ? plannedByTasks
    : (sCurveSeries ? round1(sCurveSeries.baselineNow * 100) : projectBaselinePct(selectedProject, progressPct));
  ...
  if (!grouped.length) {
    ...
    const rollupForTasks = (items) => {
      ...
      const progress = clampPct(
        list.length ? Math.round(list.reduce((sum, t) => sum + taskProgressValue(t), 0) / list.length) : 0
      );
      // Rollup rule:
    ...
    projectRow.innerHTML = `
      ...
      <div>${progressCellHtml(projectRollup.progress, false, null, toneKeyFromStatus(projectRollup.status.value || projectRollup.status.label || ""))}</div>
      <span class="pill ${projectRollup.status.className}">${projectRollup.status.label}</span>
      <span class="pill ${projectRollup.schedule.className}">${projectRollup.schedule.label}</span>
    ...
    grouped.forEach(({ phase, tasks, subEpics, isEmpty }) => {
      ...
      epicRow.innerHTML = `
        ...
        <div>${progressCellHtml(epicRollup.progress, false, null, toneKeyFromStatus(epicRollup.status.value || epicRollup.status.label || ""))}</div>
        <span class="pill ${epicRollup.status.className}">${epicRollup.status.label}</span>
        <span class="pill ${epicRollup.schedule.className}">${epicRollup.schedule.label}</span>
      ...
      if (subEpics.length && !isCollapsed) {
        ...
        const renderTaskRow = (task, level, macroLabelForTitle = "") => {
          ...
          row.innerHTML = `
            ...
            <div>${progressCellHtml(progressValue, true, task._idx, toneKeyFromStatus(getTaskStatus(task)))}</div>
            <button class="pill ${info.className} status-btn" data-task-index="${task._idx}">
              ${info.label}
            </button>
            <span class="pill ${scheduleForTask(task).className}">${scheduleForTask(task).label}</span>
            <div class="members-cell">
        ...
        const renderMacroNode = (node, level, path) => {
          ...
          macroRow.innerHTML = `
            ...
            <div>${progressCellHtml(macroRollup.progress, false, null, toneKeyFromStatus(macroRollup.status.value || macroRollup.status.label || ""))}</div>
            <span class="pill ${macroRollup.status.className}">${macroRollup.status.label}</span>
            <span class="pill ${macroRollup.schedule.className}">${macroRollup.schedule.label}</span>
            <div class="members-cell">
              ${groupMembersHtml(phase)}
...
function wireModals() {
  ...
  if (projectForm) {
    projectForm.addEventListener("submit", (e) => {
      ...
      if (db) {
        ...
        const newProject = { ...payload, squadId: payload.squadId || "", team: payload.team || [], tasks: [], epics: DEFAULT_EPICS.slice(), packages: [], financials: [] };
  ...
  if (improvementForm) {
    improvementForm.addEventListener("submit", async (e) => {
      ...
      const origin = originRaw === "chamado" ? "chamado" : "email";
...
async function openImportCronograma() {
  ...
  if (!sheet) {
    alert("Planilha vazia ou invalida.");
    return;
  }

  // Force date formatting to ISO-like strings when possible; reduces locale/serial surprises.
  ...
  if (!importedRaw.length) {
    alert(
      "Nenhuma linha importada. Confirme se o XLSX possui as colunas: Nome (Atividade/Estrutura), Inicio, Fim (ou Termino), %Concluido (Progresso) e Nomes dos recursos (Membros)."
    );
    return;
  }

  // Epicos sao fixos na plataforma: nao criamos novos epicos via importacao.
  const allowedEpics = Array.isArray(project.epics) && project.epics.length ? project.epics : DEFAULT_EPICS.slice();
  const { tasks: imported, unknownEpics } = resolveImportedTasksForFixedEpics(importedRaw, allowedEpics);
  if (!imported.length) {
    alert("Nada para importar (as linhas encontradas parecem ser apenas cabecalhos de EPICO).");
    return;
  }
...
function openActivityModal(mode = "new", taskIndex = null, defaults = null) {
  ...
  if (mode === "edit" && Number.isInteger(taskIndex) && state.selectedProject) {
    ...
    if (defaults && Object.prototype.hasOwnProperty.call(defaults, "package")) {
      ...
      if (forcedPackage && PACKAGE_PHASES.includes((form.elements.phase.value || "").toUpperCase())) {
...
function exportImprovementsReportPdf(data) {
  ...
  const addKpis = () => {
    const kpis = [
      ...
      { label: "SUCESSO MENSAL", val: "94%", icon: "trend", border: [16, 185, 129] }
    ];
...
function openImprovementsReportModal() {
  ...
  const updateKpis = (list) => {
    ...
    const avg = total ? Math.round(list.reduce((acc, item) => acc + Number(item.progress || 0), 0) / total) : 0;
...
function openImprovementModal(improvementId = null) {
  ...
  if (form.elements.origin) form.elements.origin.value = improvement?.origin || "email";
  ...
  if (form.elements.stage) form.elements.stage.value = normalizeImprovementStage(improvement?.stage || improvement?.status || "triagem");
  ...
  if (codeEl) codeEl.textContent = improvement ? improvementDisplayId(improvement) : "Nova";
  const projEl = modal.querySelector("[data-improvement-project]");
  if (projEl) projEl.textContent = improvement ? (improvement.client || "-") : "-";
...
function resetImprovementModal() {
  ...
  if (modal) {
    ...
    const codeEl = modal.querySelector("[data-improvement-code]");
    if (codeEl) codeEl.textContent = "IMP";
    const projEl = modal.querySelector("[data-improvement-project]");
...
function setupTaskActions() {
  ...
  deleteBtn.addEventListener("click", () => {
    if (popover.dataset.mode === "epic") {
      const phase = normalizePhaseLabel(popover.dataset.epicPhase || "");
      if (!phase) return;
      const isDefault = DEFAULT_EPICS.map((x) => normalizePhaseLabel(x)).includes(phase);
      if (isDefault) {
        alert("Nao e possivel excluir epicos padrao (LEVANTAMENTO/DESENVOLVIMENTO/TESTES/DEPLOY).");
        return;
      }
  ...
  if (createMacroBtn) {
    createMacroBtn.addEventListener("click", async () => {
      ...
      const phase = normalizePhaseLabel(task.phase || "OUTROS");
      if (!PACKAGE_PHASES.includes(phase)) {
        alert(`Macros/pacotes estao disponiveis apenas em: ${PACKAGE_PHASES.join(", ")}.`);
        return;
      }
      const name = promptNewPackageName();
      if (!name) return;
...
function setupTaskDragAndDrop() {
  ...
  const applyMove = async (source, target, ev = null) => {
    ...
    if (source.kind === "epic") {
      ...
      const list = Array.isArray(project.epics) && project.epics.length ? project.epics.slice() : DEFAULT_EPICS.slice();
...
function openTaskActionsPopover(target) {
  ...
  if (isEpic) {
    ...
    const phase = normalizePhaseLabel(task?.phase || "OUTROS");
    const canCreateMacro = !hasPackage && PACKAGE_PHASES.includes(phase);
...
function allProjects() {
  return state.clients.flatMap((client) =>
    (client.projects || []).map((p) => {
      ...
      const baseline = plannedByTasks != null
        ? plannedByTasks
